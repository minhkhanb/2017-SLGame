...
//#endif
	...
	//int beginAngle = 90 + 200;
	int beginAngle = 90 + 200 + 40;
	_opacityItems = 0;
	for (int i = 0; i < SETTING_ITEMS_NUMS -1; i++)
	{
#if defined (ANDROID_OS) || _WINDOWS
		if (i == CSetting::enumSettingItems::SETTING_REMOVE_ADS || i == CSetting::enumSettingItems::SETTING_ICLOUD || i == CSetting::enumSettingItems::SETTING_RESTORE)
		{
			continue;
		}
#endif
		...
		case CSetting::enumSettingItems::SETTING_ICLOUD:
			_settingItems[i]._button.SetAnim(SPRITEID_HUD_SETTING, 16);
			_settingItems[i]._frame = 13;
			beginAngle += 40;
		...
		case CSetting::enumSettingItems::SETTING_RESTORE:
			_settingItems[i]._button.SetAnim(SPRITEID_HUD_SETTING, 28);
			_settingItems[i]._frame = 37;
			beginAngle += 40;
			_settingItems[i]._angleCheck += 18;
			break;
		...
		case CSetting::enumSettingItems::SETTING_REMOVE_ADS:
			_settingItems[i]._button.SetAnim(SPRITEID_HUD_SETTING, 24);
			_settingItems[i]._frame = 27;
			beginAngle += 40;
			_settingItems[i]._angleCheck += 25;
			break;
	...
	if (!_isRenderCredit)
	{
		for (int i = 0; i < SETTING_ITEMS_NUMS - 1; i++)
		{
			...
			if (i == _currentItems)
			{
				...
				{					
					if (i== CSetting::enumSettingItems::SETTING_REMOVE_ADS && _game->_user._removeAds)
					{
						_settingItems[i]._button.SetState(BUTTON_STATE_LOCK);
					}
					else if (i == CSetting::enumSettingItems::SETTING_COLOR && !_game->_user._isUnlockColor)
					{
						_settingItems[i]._button.SetState(BUTTON_STATE_LOCK);
					}
					else
...
void CSetting::RenderItems(float x, float y)
{
	...
	for (int i = 0; i < SETTING_ITEMS_NUMS-1; i++)
	{
		...
		else if (i == CSetting::enumSettingItems::SETTING_REMOVE_ADS
			&& !_settingItems[i]._button.CheckState(BUTTON_STATE_HIDE)
			&& !_settingItems[i]._button.CheckState(BUTTON_STATE_LOCK))
		{
			...
			for (int touch = 0; touch < SETTING_ITEMS_NUMS; touch++)
			{
				...
				if (CHECK_POINT_IN_RECT(TOUCHS(i)->x, TOUCHS(i)->y, x, y, w, h))
				{
					if (TOUCHS(i)->_state == TOUCH_DOWN)
					{
						...
						//PDEBUG("\n DOWN touch id:%d -- x:%d -- y:%d -- w:%d -- h:%d", touch, x, y, w, h);
					}
